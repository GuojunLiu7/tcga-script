...
def extract_filter(y, t, ARS, total_matrix, summary_index, tumor_name):
	...
	summary_index = save_fig(summary_index, matrix_male, gene_matrix, t, ARS, "Primary Tumor: Male", "gender_male", tumor_name)
	matrix_female = matrix_primary[matrix_primary['gender'] == "female"]
	summary_index = save_fig(summary_index, matrix_female, gene_matrix, t, ARS, "Primary Tumor: Female", "gender_female", tumor_name)
	#=====================================
	...
	summary_index = save_fig(summary_index, matrix_i, gene_matrix, t, ARS, "Primary Tumor: Stage I", "primary_tumor_stage_i", tumor_name)
	...
	summary_index = save_fig(summary_index, matrix_ii, gene_matrix, t, ARS, "Primary Tumor: Stage II", "primary_tumor_stage_ii", tumor_name)	
	matrix_iii = matrix_primary[matrix_primary['tumor_stage'].str.startswith('stage iii') == True]
	summary_index = save_fig(summary_index, matrix_iii, gene_matrix, t, ARS, "Primary Tumor: Stage III", "primary_tumor_stage_iii", tumor_name)
	matrix_iv = matrix_primary[matrix_primary['tumor_stage'].str.startswith('stage iv') == True]
	summary_index = save_fig(summary_index, matrix_iv, gene_matrix, t, ARS, "Primary Tumor: Stage IV", "primary_tumor_stage_iv", tumor_name)
		...
		for ele in d:
			...
			for item in category:
				if (category[item] > 10) and (item != "NotApplicable") and (len(category) < 10):
					...
					summary_index = save_fig(summary_index, matrix_ele, gene_matrix, t, ARS, ("Primary Tumor: " + ele.replace("_", " ").title() + " " + item), (ele + "_" + (item.replace(" ", "_").replace("/", "_")).lower()), tumor_name)
	return(summary_index)
